...
// functions tied to a KEY TYPE -> VALUE TYPE.
//
// ??? HOW TO USE ???
...
//         return 1;
//     }
//     int_to_string_hmap_put(&map, 69, "Six Nine", &err);
//     if(err.code != HMAP_OK) {
//         printf("Error: %s\n", err.message);
//         return 1;
//     }
//     char* value = int_to_string_hmap_get_or_null(&map, 69, &err);
//     if(err.code != HMAP_OK) {
//         printf("Error: %s\n", err.message);
//         return 1;
//     }
//     printf("%s", value);
//     int_to_string_hmap_clear(&map);
//     int_to_string_hmap_free(&map);
//     return 0;
// }
//
//
...
#define hmap_resize(map_ptr, new_capacity, out_error_ptr)   ((map_ptr)->op->_resize(map_ptr, new_capacity, out_error_ptr))
#define hmap_reserve(map_ptr, new_capacity, out_error_ptr)  ((map_ptr)->op->_reserve(map_ptr, new_capacity, out_error_ptr))
#define hmap_try_shrink(map_ptr, out_error_ptr)             ((map_ptr)->op->_try_shrink(map_ptr, out_error_ptr))
#define hmap_free(map_ptr)                                  ((map_ptr)->op->_free(map_ptr))
#define hmap_put(map_ptr, key, element, out_error_ptr)      ((map_ptr)->op->_put(map_ptr, key, element, out_error_ptr))
#define hmap_get_or_null(map_ptr, key, out_error_ptr)       ((map_ptr)->op->_get_or_null(map_ptr, key, out_error_ptr))
#define hmap_remove(map_ptr, key, out_error_ptr)            ((map_ptr)->op->_remove(map_ptr, key, out_error_ptr))
#define hmap_contains(map_ptr, key, out_error_ptr)          ((map_ptr)->op->_contains(map_ptr, key, out_error_ptr))
#define hmap_clear(map_ptr)                                 ((map_ptr)->op->_clear(map_ptr))
#define hmap_clone(map_ptr, map_clone_ptr, out_error_ptr)   ((map_ptr)->op->_clone(map_ptr, map_clone_ptr, out_error_ptr))
#define hmap_next(map_ptr, iterator_ptr, out_error_ptr)     ((map_ptr)->op->_next(map_ptr, iterator_ptr, out_error_ptr))
#define hmap_iterator(map_ptr, iterator_ptr)                ((map_ptr)->op->_iterator(iterator_ptr))

#ifdef HMAP_STRIP_PREFIXES

#define resize(map_ptr, new_capacity, out_error_ptr)   hmap_resize(map_ptr, new_capacity, out_error_ptr)
#define reserve(map_ptr, new_capacity, out_error_ptr)  hmap_reserve(map_ptr, new_capacity, out_error_ptr)
#define try_shrink(map_ptr, out_error_ptr)             hmap_try_shrink(map_ptr, out_error_ptr)
#define put(map_ptr, key, element, out_error_ptr)      hmap_put(map_ptr, key, element, out_error_ptr)
#define get_or_null(map_ptr, key, out_error_ptr)       hmap_get_or_null(map_ptr, key, out_error_ptr)
#define remove(map_ptr, key, out_error_ptr)            hmap_remove(map_ptr, key, out_error_ptr)
#define contains(map_ptr, key, out_error_ptr)          hmap_contains(map_ptr, key, out_error_ptr)
#define clear(map_ptr)                                 hmap_clear(map_ptr)
#define clone(map_ptr, map_clone_ptr, out_error_ptr)   hmap_clone(map_ptr, map_clone_ptr, out_error_ptr)
#define next(map_ptr, iterator_ptr, out_error_ptr)     hmap_next(map_ptr, iterator_ptr, out_error_ptr)
#define iterator(map_ptr, iterator_ptr)                hmap_iterator(map_ptr, iterator_ptr)

#endif

// THE IMPLEMENTATION:
enum HMAP_ERROR_CODES {
    HMAP_OK,
    HMAP_ALLOC,
    HMAP_NOT_FOUND,
    HMAP_MEM_CORRUPTION
...
typedef struct hmap_error_t {
    const char* message;
    enum HMAP_ERROR_CODES code;
} hmap_error_t;

#define STR_(x) #x
#define STR(x) STR_(x)

#define hmap_err(code_, string_literal, func) ((hmap_error_t){.message = (__FILE__"(line: "STR(__LINE__)") in "STR(func)"(...): "string_literal), .code = (code_)})
static inline hmap_error_t hmap_ok() { return (hmap_error_t) {.message = 0, .code = HMAP_OK}; }
...
//   #######################################################
#define _PUT_WITHOUT_SIZE_CHECK_(map, k, v, KEY_T, out_error, func) do {                                                                \
    ...
    for (size_t probe = 0; probe < map->capacity; probe++) {                                                                            \
        ...
        if (state == 1) {                                                                                                               \
            ...
            *out_error = hmap_err(HMAP_MEM_CORRUPTION, "Invalid bitmap state (3) detected", func);                                      \
            return;                                                                                                                     \
        }                                                                                                                               \
    }                                                                                                                                   \
    if(first_deleted != SIZE_MAX) {                                                                                                     \
...
struct MAP_STRUCT_NAME##_operations {                                                                                                   \
    void(*const _resize)(MAP_STRUCT_NAME*, size_t, hmap_error_t*);                                                                      \
    void(*const _reserve)(MAP_STRUCT_NAME*, size_t, hmap_error_t*);                                                                     \
    bool(*const _try_shrink)(MAP_STRUCT_NAME*, hmap_error_t*);                                                                          \
    void(*const _free)(MAP_STRUCT_NAME*);                                                                                               \
    void(*const _put)(MAP_STRUCT_NAME*, KEY_T, VALUE_T, hmap_error_t*);                                                                 \
    VALUE_T*(*const _get_or_null)(MAP_STRUCT_NAME*, KEY_T, hmap_error_t*);                                                              \
    void(*const _remove)(MAP_STRUCT_NAME*, KEY_T, hmap_error_t*);                                                                       \
    bool(*const _contains)(MAP_STRUCT_NAME*, KEY_T, hmap_error_t*);                                                                     \
    void(*const _clear)(MAP_STRUCT_NAME*);                                                                                              \
    void(*const _clone)(MAP_STRUCT_NAME*, MAP_STRUCT_NAME*, hmap_error_t*);                                                             \
    MAP_STRUCT_NAME##_entry*(*const _next)(MAP_STRUCT_NAME*, MAP_STRUCT_NAME##_iterator*, hmap_error_t*);                                              \
    MAP_STRUCT_NAME##_iterator(*const _iterator)(MAP_STRUCT_NAME##_iterator*);                                                          \
};                                                                                                                                      \
                                                                                                                                        \
static void MAP_STRUCT_NAME##_init(MAP_STRUCT_NAME* map, hmap_error_t* out_error);                                                      \
                                                                                                                                        \
/* capacity must be a power of 2 */                                                                                                     \
static void MAP_STRUCT_NAME##_init_with_capacity(MAP_STRUCT_NAME* map, size_t capacity, hmap_error_t* out_error);                       \
                                                                                                                                        \
/* capacity must be a power of 2 */                                                                                                     \
static void MAP_STRUCT_NAME##_resize(MAP_STRUCT_NAME* map, size_t new_capacity, hmap_error_t* out_error) {                              \
    ...
    if (!map->elements) {                                                                                                               \
        *out_error = hmap_err(HMAP_ALLOC, "Failed to allocate elements", MAP_STRUCT_NAME##_resize);                                     \
        return;                                                                                                                         \
    }                                                                                                                                   \
                                                                                                                                        \
    ...
    if (!map->bitmap) {                                                                                                                 \
        ...
        *out_error = hmap_err(HMAP_ALLOC, "Failed to allocate bitmap", MAP_STRUCT_NAME##_resize);                                       \
        return;                                                                                                                         \
    }                                                                                                                                   \
                                                                                                                                        \
    ...
    for (size_t i = 0; i < old_capacity; i++) {                                                                                         \
        ...
        if (state == 1) {                                                                                                               \
            ...
            _PUT_WITHOUT_SIZE_CHECK_(map, k, v, KEY_T, out_error, MAP_STRUCT_NAME##_resize);                                            \
            if(out_error->code != HMAP_OK) {                                                                                            \
                free(old_elements);                                                                                                     \
                free(old_bitmap);                                                                                                       \
                return;                                                                                                                 \
            }                                                                                                                           \
    ...
    *out_error = hmap_ok();                                                                                                             \
                                                                                                                                        ...
                                                                                                                                        \
static void MAP_STRUCT_NAME##_reserve(MAP_STRUCT_NAME* map, size_t new_capacity, hmap_error_t* out_error) {                             \
    assert(map != NULL && "A valid map is expected");                                                                                   \
    if(map->capacity < new_capacity) {                                                                                                  \
        MAP_STRUCT_NAME##_resize(map, nextPowerOfTwo(new_capacity), out_error);                                                         \
    } else {                                                                                                                            \
        *out_error = hmap_ok();                                                                                                         \
    }                                                                                                                                   \
}                                                                                                                                       \
                                                                                                                                        \
static bool MAP_STRUCT_NAME##_try_shrink(MAP_STRUCT_NAME* map, hmap_error_t* out_error) {                                               \
    assert(map != NULL && "A valid map is expected");                                                                                   \
    size_t new_capacity = nextPowerOfTwo((size_t)(map->size/LOAD_FACTOR));                                                              \
    if(map->capacity > new_capacity) {                                                                                                  \
        MAP_STRUCT_NAME##_resize(map, new_capacity, out_error);                                                                         \
        return out_error->code == HMAP_OK;                                                                                              \
    }                                                                                                                                   \
    *out_error = hmap_ok();                                                                                                             \
    return false;                                                                                                                       \
}                                                                                                                                       \
...
static void MAP_STRUCT_NAME##_free(MAP_STRUCT_NAME* map) {                                                                              \
    ...
    map->capacity = 0;                                                                                                                  \
                                                                                                                                        ...
                                                                                                                                        \
static void MAP_STRUCT_NAME##_put(MAP_STRUCT_NAME* map, KEY_T key, VALUE_T value, hmap_error_t* out_error) {                            \
    assert(map != NULL && "A valid map is expected");                                                                                   \
    if (map->size >= (size_t)(map->capacity * LOAD_FACTOR)) {                                                                           \
        MAP_STRUCT_NAME##_resize(map, map->capacity * 2, out_error);                                                                    \
        if(out_error->code != HMAP_OK) return;                                                                                          \
    }                                                                                                                                   \
    _PUT_WITHOUT_SIZE_CHECK_(map, key, value, KEY_T, out_error, MAP_STRUCT_NAME##_put);                                                 \
}                                                                                                                                       \
                                                                                                                                        \
static VALUE_T* MAP_STRUCT_NAME##_get_or_null(MAP_STRUCT_NAME* map, KEY_T key, hmap_error_t* out_error) {                               \
    assert(map != NULL && "A valid map is expected");                                                                                   \
    *out_error = hmap_ok();                                                                                                             \
    ...
    while (counter--) {                                                                                                                 \
        ...
        if (state == 1) {                                                                                                               \
            ...
            *out_error = hmap_err(HMAP_MEM_CORRUPTION, "Invalid bitmap state (0b11) detected", MAP_STRUCT_NAME##_get_or_null);          \
            return NULL;                                                                                                                \
        }                                                                                                                               \
                                                                                                                                        \
        bit_index += 2;                                                                                                                 \
    ...
    return NULL;                                                                                                                        \
                                                                                                                                        ...
                                                                                                                                        \
static void MAP_STRUCT_NAME##_remove(MAP_STRUCT_NAME* map, KEY_T key, hmap_error_t* out_error) {                                        \
    assert(map != NULL && "A valid map is expected");                                                                                   \
    *out_error = hmap_ok();                                                                                                             \
    if (map->size == 0)                                                                                                                 \
        return;                                                                                                                         \
    ...
    while (counter--) {                                                                                                                 \
        ...
        if (state == 1) {                                                                                                               \
            ...
            *out_error = hmap_err(HMAP_MEM_CORRUPTION, "Invalid bitmap state (3) detected", MAP_STRUCT_NAME##_remove);                  \
            return;                                                                                                                     \
        }                                                                                                                               \
                                                                                                                                        \
        bit_index += 2;                                                                                                                 \
        if (bit_index >= CHAR_BIT) {                                                                                                    \
            ...
            byte_index = (byte_index + 1) & (map->capacity / SIZEOF_BITMAP - 1);                                                        \
                                                                                                                                        ...
                                                                                                                                        \
static bool MAP_STRUCT_NAME##_contains(MAP_STRUCT_NAME* map, KEY_T key, hmap_error_t* out_error) {                                      \
    assert(map != NULL && "A valid map is expected");                                                                                   \
    *out_error = hmap_ok();                                                                                                             \
    ...
    while (counter--) {                                                                                                                 \
        ...
        if (state == 1) {                                                                                                               \
            ...
            *out_error = hmap_err(HMAP_MEM_CORRUPTION, "Invalid bitmap state (3) detected", MAP_STRUCT_NAME##_contains);                \
            return false;                                                                                                               \
        }                                                                                                                               \
                                                                                                                                        \
        bit_index += 2;                                                                                                                 \
...
static void MAP_STRUCT_NAME##_clear(MAP_STRUCT_NAME* map) {                                                                             \
    ...
    memset(map->bitmap, 0, (map->capacity + SIZEOF_BITMAP - 1) / SIZEOF_BITMAP);                                                        \
                                                                                                                                        ...
                                                                                                                                        \
static void MAP_STRUCT_NAME##_clone(MAP_STRUCT_NAME* map1, MAP_STRUCT_NAME* map2, hmap_error_t* out_error) {                            \
    assert(map1 != NULL && "A valid source map is expected");                                                                           \
    assert(map2 != NULL && "A valid source map is expected");                                                                           \
    MAP_STRUCT_NAME##_init_with_capacity(map2, map1->capacity, out_error);                                                              \
    if(out_error->code != HMAP_OK) return;                                                                                              \
    if(map1->size == 0) {                                                                                                               \
        MAP_STRUCT_NAME##_init(map2, out_error);                                                                                        \
        return;                                                                                                                         \
    }                                                                                                                                   \
    map2->size = map1->size;                                                                                                            \
    memcpy(map2->bitmap, map1->bitmap, (map1->capacity + SIZEOF_BITMAP - 1) / SIZEOF_BITMAP);                                           \
    memcpy(map2->elements, map1->elements, map1->capacity * sizeof(MAP_STRUCT_NAME##_entry));                                           \
    *out_error = hmap_ok();                                                                                                             \
}                                                                                                                                       \
                                                                                                                                        \
static MAP_STRUCT_NAME##_iterator MAP_STRUCT_NAME##_iterator_init(MAP_STRUCT_NAME##_iterator* it) {                                     \
    ...
    return *it;                                                                                                                         \
                                                                                                                                        ...
                                                                                                                                        \
static MAP_STRUCT_NAME##_entry* MAP_STRUCT_NAME##_next(MAP_STRUCT_NAME* map, MAP_STRUCT_NAME##_iterator* it, hmap_error_t* out_error) { \
    assert(map != NULL && "A valid map is expected");                                                                                   \
    if(map->size == 0) return NULL;                                                                                                     \
...
static void MAP_STRUCT_NAME##_init(MAP_STRUCT_NAME* map, hmap_error_t* out_error) {                                                     \
    ...
    if (!map->elements) {                                                                                                               \
        *out_error = hmap_err(HMAP_ALLOC, "Failed to allocate elements", MAP_STRUCT_NAME##_init);                                       \
        return;                                                                                                                         \
    }                                                                                                                                   \
    map->bitmap = (byte*)calloc(1, sizeof(byte));                                                                                       \
    if (!map->bitmap) {                                                                                                                 \
        free(map->elements);                                                                                                            \
        *out_error = hmap_err(HMAP_ALLOC, "Failed to allocate bitmap", MAP_STRUCT_NAME##_init);                                         \
        return;                                                                                                                         \
    }                                                                                                                                   \
    map->size = 0;                                                                                                                      \
    map->capacity = 4;                                                                                                                  \
...
static void MAP_STRUCT_NAME##_init_with_capacity(MAP_STRUCT_NAME* map, size_t capacity, hmap_error_t* out_error) {                      \
    ...
    if (!map->elements) {                                                                                                               \
        *out_error = hmap_err(HMAP_ALLOC, "Failed to allocate elements", MAP_STRUCT_NAME##_init_with_capacity);                         \
        return;                                                                                                                         \
    }                                                                                                                                   \
    map->bitmap = (byte*)calloc((capacity + SIZEOF_BITMAP - 1) / SIZEOF_BITMAP, sizeof(byte));                                          \
    if (!map->bitmap) {                                                                                                                 \
        free(map->elements);                                                                                                            \
        *out_error = hmap_err(HMAP_ALLOC, "Failed to allocate bitmap", MAP_STRUCT_NAME##_init_with_capacity);                           \
        return;                                                                                                                         \
    }                                                                                                                                   \
    map->size = 0;                                                                                                                      \
